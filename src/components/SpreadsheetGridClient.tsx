            ...
            return (
              <div key={i} className="flex border-b border-gray-100" style={{ height: 20 }}>
                <div className="shrink-0 border-r border-gray-200 bg-gray-50"
                  style={{ width: 202, display:"flex", alignItems:"center", paddingLeft: 8 }}>
...
export default function SpreadsheetGridClient() {
  ...
  useEffect(() => {
    const safeJson = async (url: string) => {
      ...
      if (!text) throw new Error(`${url} returned empty (${r.status})`)
      let parsed: unknown
      try { parsed = JSON.parse(text) } catch { throw new Error(`${url} non-JSON: ${text.slice(0,200)}`) }
    ...
    Promise.all([
      ...
      const [devs, asgns, tsks, settings, locs] =
        results as [DeviceInfo[], AssigneeInfo[], TaskInfo[], DisplaySettings, LocationInfo[]]
      setDevices(devs)
      setAssignees(asgns)
      setTasks(tsks)
  ...
  /* modelIds → 実際の deviceId リストに変換 */
  const visibleDeviceIds = useMemo<string[] | undefined>(() => {
    if (!displaySettings) return undefined
    const vis = new Set(displaySettings.modelIds)
    return devices.filter(d => vis.has(d.modelId)).map(d => d.id)
  }, [displaySettings, devices])
  ...
  const handleSaveSettings = async (
    modelIds: string[], assigneeIds: string[], showLocationRowInDevice: boolean
  ) => {
    await fetch("/api/display-settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ modelIds, assigneeIds, showLocationRowInDevice }),
    })
    setDisplaySettings({ modelIds, assigneeIds, showLocationRowInDevice })
    setShowDialog(false)
  }
    ...
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-white">
        ...
        <>
          <div className={`flex-1 overflow-hidden ${tab === "device" ? "flex" : "hidden"} flex-col`}>
            <SpreadsheetGrid
              ...
              visibleGroupIds={visibleDeviceIds}
              showLocationRow={displaySettings?.showLocationRowInDevice}
      ...
      {showDialog && displaySettings && (
        <DisplaySettingsDialog
          ...
          selectedModelIds={displaySettings.modelIds}
          selectedAssigneeIds={displaySettings.assigneeIds}
          showLocationRowInDevice={displaySettings.showLocationRowInDevice}
